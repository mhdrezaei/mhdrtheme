		...
		<?php elseif ( is_search() ): ?>
			<h1>
				...
				<?php $search_results=$wp_query->found_posts;
					if ($search_results==1) {
						echo $search_results.' '.esc_html__('نتایج جستجو','mywayblog');
					} else {
						echo $search_results.' '.esc_html__('نتایج جستجو','mywayblog');
					}
				?>
			...
			<div class="notebox">
<!--				--><?php //esc_html_e('For the term','mywayblog'); ?><!-- "<span>--><?php //echo get_search_query(); ?><!--</span>".-->
				<?php if ( !have_posts() ): ?>
					<?php esc_html_e('لطفا مورد دیگری را جستجو کنید:','mhdrtheme'); ?>
				<?php endif; ?>

			</div>
			
		<?php elseif ( is_404() ): ?>
			<h1><i class="fas fa-exclamation-circle"></i><?php esc_html_e('Error 404.','mywayblog'); ?> <span><?php esc_html_e('Page not found!','mywayblog'); ?></span></h1>
			<div class="notebox">	
				<p><?php esc_html_e( 'صفحه ای که می خواهید به آن دسترسی پیدا کنید موجود نمی باشد یا منتقل شده است. لطفاً از منوها یا کادر جستجو برای یافتن آنچه به دنبال آن هستید استفاده کنید.', 'mywayblog' ); ?></p>
				<?php get_search_form(); ?>
			</div>
